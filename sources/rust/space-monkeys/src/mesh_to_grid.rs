...
fn terrain_gradient(particles: &[SmoothedParticle], p: Vec2D) -> f32 {
    let d = 0.001;
    let grad_x = (terrain_height(particles, p + Vec2D { x: d, y: 0.0 }) - terrain_height(particles, p - Vec2D { x: d, y: 0.0 })) / (2.0 * d);
    let grad_y = (terrain_height(particles, p + Vec2D { x: 0.0, y: d }) - terrain_height(particles, p - Vec2D { x: 0.0, y: d })) / (2.0 * d);
...
pub fn mesh_to_grid(filename: &str, min_x: f32, max_x: f32, min_z: f32, max_z: f32, res_x: usize,
    ...
    for grid_z in 0..res_z {
        for grid_x in 0..res_x {
            let x = math::lerp_f32(min_x, max_x, grid_x as f32 / res_x as f32);
            let z = math::lerp_f32(min_z, max_z, grid_z as f32 / res_z as f32);
